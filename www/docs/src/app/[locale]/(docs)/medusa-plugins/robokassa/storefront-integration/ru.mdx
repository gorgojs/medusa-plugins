...
description: "Узнайте, как интегрировать платёжный провайдер Робокасса в витрину магазина на Next.js для Medusa."
---
...
При инициализации платёжной сессии для Робокасса параметры `SuccessUrl2` и `FailUrl2` определяют, куда будет перенаправлен покупатель после попытки оплаты. Оба URL динамически формируются на основе базового URL storefront, идентификатора корзины и выбранного кода страны.
