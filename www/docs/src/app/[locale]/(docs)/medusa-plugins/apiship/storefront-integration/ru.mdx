...
## 1. Требовать номер телефона получателя

ApiShip требует номер телефона получателя для расчета стоимости доставки и создания заказа в ApiShip. По этой причине storefront должен требовать указания номера телефона в форме адреса доставки.

Откройте [`src/modules/checkout/components/shipping-address/index.tsx`](https://github.com/ gorgojs/medusa-plugins/blob/7aa21a2d0331f60495ad85be74fcaa1aa86dd2a5/examples/fulfillment-apiship/medusa-storefront/src/modules/checkout/components/shipping-address/index.tsx#L212) и отметьте поле для ввода номера телефона как обязательное:

![Структура каталога в Medusa Storefront после обновления файла для компонента адреса доставки](https://github.com/user-attachments/assets/08552cbd-f3f3-4526-a334-b86b66f48e37)
...
Пометив поле ввода номера телефона как `required`, система оформления заказа всегда собирает номер телефона получателя в начале процесса, обеспечивая наличие необходимых данных для расчета стоимости доставки ApiShip и запросов на создание отправления.

## 2. Обработка данных корзины

Для интеграции ApiShip в процесс оформления заказа, storefront должен иметь возможность сохранять выбор клиента по доставке в корзине и извлекать его позже. Это обеспечивает стабильность процесса при обновлении страницы и навигации. Если клиент уже выбрал тариф и пункт самовывоза, storefront может восстановить этот выбор, вместо того чтобы просить его выбрать заново. Интеграция также требует надлежащего действия по удалению, которое очищает выбор не только визуально, но и в самой корзине.

Откройте [`src/lib/data/cart.ts`](https://github.com/gorgojs/medusa-plugins/blob/7aa21a2d0331f60495ad85be74fcaa1aa86dd2a5/examples/fulfillment-apiship/medusa-storefront/src/lib/data/cart.ts# L217-L242) и добавьте следующее:

![Структура каталога в Medusa Storefront после обновления файла для корзины](https://github.com/user-attachments/assets/f2563a45-53bd-4e28-8d25-8b612d0bb8ba)

```ts
export async function retrieveCart(cartId?: string, fields?: string) {
  // ...
  // Добавьте +shipping_methods.data parameter
...
- Storefront может прикреплять данные выбора, специфичные для ApiShip, при применении способа доставки.
- Он может считывать сохраненные данные из корзины при перезагрузке страницы.
- Он может полностью очистить выбор, удалив способ доставки из корзины, когда клиент решает сбросить свой выбор.

## 3. Получение данных ApiShip

Для правильного отображения вариантов доставки ApiShip на этапе доставки, storefront должен иметь доступ к динамическим данным калькульляции, информации о пунктах самовывоза и списку доступных поставщиков. Помимо тарифов и пунктов самовывоза, storefront также получает метаданные поставщиков, чтобы отображать в интерфейсе оформления заказа понятные для человека названия перевозчиков вместо внутренних идентификаторов.
...
![Структура каталога в Medusa Storefront после обновления файла для выполнения заказов](https://github.com/user-attachments/assets/c37d99e7-403a-4fef-84dc-ed351087a4dc)
...
Запрос `retrieveCalculation` возвращает доступные тарифы доставки вместе с идентификаторами пунктов самовывоза, к которым они относятся. Запрос `getPointAddresses` преобразует эти идентификаторы в полные данные о пунктах самовывоза, такие как адреса и координаты, которые затем могут быть отображены в интерфейсе оформления заказа.

Кроме того, storefront загружает список поставщиков ApiShip, чтобы отображать в интерфейсе удобные для человека названия перевозчиков.

## 4. Определения типов

Интеграция со storefront основана на небольшом наборе общих типов, которые описывают данные, возвращаемые конечной точкой вычисления ApiShip, и данные, сохраняемые в корзине в качестве выбора пользователя по доставке. Эти типы используются в интерфейсе карты, модальных окнах курьеров и пунктов самовывоза, а также в выбранном компоненте для обеспечения согласованности потока и типовой безопасности.
...
![Структура каталогов в Medusa Storefront после создания файла для типов ApiShip](https://github.com/user-attachments/assets/759f8e54-1f6b-4371-a518-78bddcdcf0f1)
...
`ApishipCalculation` представляет рассчитанные варианты доставки, возвращаемые API store, разделенные на группы `to door` и `to point` по поставщикам. `Chosen` — это нормализованная полезная нагрузка, которую storefront сохраняет в корзине после того, как пользователь выбирает тариф и пункт самовывоза.

## 5. Утилиты

Интеграция со storefront использует набор общих утилит для обеспечения единообразия пользовательского интерфейса и предотвращения дублирования логики в компонентах. Эти утилиты обрабатывают форматирование времени доставки, стабильные ссылки обратного вызова для асинхронных эффектов, детерминированные ключи тарифов для выбора, блокировку прокрутки для модальных окон и отображение тарифов пунктов самовывоза в структуре, которую легко визуализировать.
...
![Структура каталогов в Medusa Storefront после создания файла для утилит](https://github.com/user-attachments/assets/a59ef69b-72e1-4dba-b094-793e39d978ef)
...
Эти утилиты используются модальными окнами и картой для форматирования сроков доставки, обеспечения стабильности ключей выбора между рендерами, безопасного запуска асинхронных эффектов без обновления состояния после отключения и правильной подготовки модели данных пунктов самовывоза, используемой для рендеринга тарифов по пунктам.

## 6. Карта пунктов самовывоза

Этот компонент отвечает за отображение карты Яндекса, размещение на ней маркеров пунктов самовывоза и показ панели с подробной информацией о доступных тарифах при выборе пункта.
...
![Структура каталогов в Medusa Storefront после создания файла карты пунктов самовывоза](https://github.com/user-attachments/assets/bffa1d01-eaf7-4c54-93bb-927b34a139a7)
...
Этот компонент инициализирует Yandex Maps v3 один раз, перецентрирует карту при изменении списка пунктов самовывоза и отображает кликабельные маркеры для каждого доступного пункта. Когда пользователь выбирает пункт, в storefront появляется боковая панель, на которой отображаются тарифы и можно выбрать один из них. Пользовательский интерфейс карты также учитывает ранее сохраненный выбор. Если пользователь снова открывает модальное окно, компонент выделяет уже выбранный пункт самовывоза и тариф и не позволяет повторно выбрать тот же вариант.

## 7. Сводка доставки ApiShip

Этот компонент отображает краткую сводку вариантов доставки, которые уже выбрал клиент. Она отображается после того, как пользователь выбрал пункт самовывоза или тариф курьера, чтобы на странице оформления заказа было четко видно, что в данный момент находится в корзине.
...
![Структура каталогов в Medusa Storefront после создания файла для ApiShip](https://github.com/user-attachments/assets/898bba01-7197-461c-89fe-ce97682b834c)
...
После этого storefront может отображать ранее сохраненный выбор непосредственно на этапе доставки, включая пункт самовывоза и тарифные детали, если применимо, и позволить пользователю либо удалить его, либо вновь открыть модальное окно, чтобы изменить его.

## 8. Модальное окно «Пункт самовывоза»

Этот компонент реализует специальное модальное окно, которое позволяет клиенту выбрать пункт самовывоза ApiShip и соответствующий тариф прямо на карте. Модальное окно отвечает за загрузку пунктов самовывоза для выбранного варианта доставки, восстановление ранее сохраненного выбора (если таковой имеется) и сохранение нового выбора в корзине после его подтверждения клиентом.
...
![Структура каталогов в Medusa Storefront после создания файла для модального окна пункта самовывоза ApiShip](https://github.com/user-attachments/assets/ffb59344-83e4-4b02-a437-58d4660e2e63)
...
Это модальное окно блокирует прокрутку тела страницы, пока оно открыто, извлекает пункты самовывоза на основе результатов расчета стоимости доставки и передает ранее сохраненный выбор на карту, чтобы интерфейс мог отображать то, что уже выбрал клиент. Когда пользователь подтверждает пункт и тариф, выбор сохраняется в данных о способе доставки в корзине, и обновленная цена пересчитывается.

## 9. Модальное окно выбора тарифа курьерской доставки
...
Когда модальное окно открыто, оно восстанавливает ранее выбранный тариф (если он уже был сохранен в корзине), поэтому пользователь сразу видит свой предыдущий выбор. Все доступные тарифы курьерской доставки отображаются в сгруппированном виде, и клиент может выбрать один из них с помощью переключателей.
...
![Структура каталогов в Medusa Storefront после создания файла для модального окна курьера ApiShip](https://github.com/user-attachments/assets/cbe90ee1-11af-46ce-bdb3-5eee32617c34)
...
После того, как клиент подтверждает свой выбор, нажимая «Выбрать», выбранный тариф сохраняется в корзине, стоимость доставки пересчитывается, и модальное окно закрывается.

## 10. Публичный экспорт

Все компоненты и вспомогательные утилиты ApiShip экспортируются через общий файл, который действует как единая точка входа для интеграции. Он повторно экспортирует основные строительные блоки пользовательского интерфейса вместе с общими типами и вспомогательными утилитами, чтобы другие части storefront могли импортировать все из одного места, а не ссылаться на внутренние пути к файлам.
...
![Структура каталогов в Medusa Storefront после создания файла для единой точки входа для интеграции](https://github.com/user-attachments/assets/39d446d4-2708-49ce-9f51-d40b6eeed470)
...
Это позволяет сохранить чистоту и согласованность импортов, а также упрощает обслуживание и рефакторинг интеграции в будущем без изменения путей импорта в storefront.

## 11. Интеграция этапа доставки

Для поддержки методов доставки, рассчитанных ApiShip, на этапе доставки требуется дополнительный интерфейс, в котором клиент выбирает конкретный тариф и, при желании, пункт самовывоза. Это обновление добавляет специфические для ApiShip состояния и модальные окна, загружает названия поставщиков для отображения и сохраняет окончательный выбор в корзине, чтобы оформление заказа могло быть продолжено только после того, как клиент сделал действительный выбор.
...
![Структура каталогов в Medusa Storefront после обновления файла для компонента доставки](https://github.com/user-attachments/assets/b526af99-7356-4bb6-ba42-12f2442aac99)
...
Это изменение добавляет два специальных модальных окна для ApiShip (выбор пункта самовывоза и выбор тарифа курьера) и небольшой блок с кратким описанием, который показывает текущий сохраненный выбор с действиями «Изменить» и «Удалить». Оно также гарантирует:

- Клиент не может продолжить оплату варианта доставки ApiShip, пока в корзине не будет сохранен действительный выбор ApiShip.
- Рассчитанные цены ApiShip отображаются как «от X», чтобы отразить, что окончательная сумма зависит от выбранного тарифа.
