...
Селектор выбора пункта выдачи заказов использует [Yandex Maps JS API v3](https://yandex.com/maps-api/docs/js-api/index.html). Чтобы включить карту в storefront, необходимо задать публичный ключ API через переменные окружения.
...
![Структура проекта storefront Medusa после обновления файла корзины](https://github.com/user-attachments/assets/f2563a45-53bd-4e28-8d25-8b612d0bb8ba)
...
![Структура проекта storefront Medusa после обновления файла для фулфилмента](https://github.com/user-attachments/assets/c37d99e7-403a-4fef-84dc-ed351087a4dc)
...
![Структура проекта storefront Medusa после обновления файла для компонента адреса доставки](https://github.com/user-attachments/assets/08552cbd-f3f3-4526-a334-b86b66f48e37)
...
![Структура проекта storefront Medusa после создания файла для компонента выбора пункта выдачи](https://github.com/user-attachments/assets/9cd4701f-3607-4c48-bd04-beaaef1d747a)
...
## 5. Компонент карты пунктов выдачи

Для выбора пункта выдачи на этапе доставки storefront требуется UI-компонент, который визуализирует доступные пункты и позволяет выбрать тариф для конкретного пункта. Этот компонент реализован как клиентский компонент на основе Yandex Maps JS API v3 и UI-компонентов Medusa.
...
![Структура проекта storefront Medusa после создания файла для компонента карты пунктов выдачи](https://github.com/user-attachments/assets/ddbedd6c-3e76-4912-b0f6-3b776e252edd)
...
Компонент `ApishipMap` отображает пункты выдачи в виде маркеров на карте и открывает панель с подробной информацией о выбранном пункте. В панели отображаются доступные тарифы для этого пункта, включая стоимость и срок доставки, а подтверждение выбора выполняется через колбэк `onChoose`, который передаёт выбранный пункт и тариф обратно в процесс оформления заказа.

## 6. Интеграция на этапе доставки
...
![Структура проекта storefront Medusa после обновления файла этапа доставки](https://github.com/user-attachments/assets/b526af99-7356-4bb6-ba42-12f2442aac99)
