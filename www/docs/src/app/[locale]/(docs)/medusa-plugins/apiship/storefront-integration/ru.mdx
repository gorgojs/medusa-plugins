...
description: "Узнайте, как интегрировать провайдер фулфилмента ApiShip в витрину магазина (Storefront) на Next.js для Medusa."
---

# {frontmatter.title}

Чтобы интегрировать провайдер фулфилмента ApiShip в витрину магазина (Storefront) Next.js, необходимо расширить этап оформления заказа, чтобы обеспечить поддержку доставки в пункт выдачи закаков.
...
Эти данные должны динамически загружаться из бэкэнда, представляться клиенту в удобной форме, а затем сохраняться в корзине, чтобы их можно было использовать позже провайдером фулфилмент при создании заказа в ApiShip.

## Переменные окружения

Выбора пункта выдачи заказов использует [Yandex Maps JS API v3](https://yandex.ru/maps-api/docs/js-api/index.html?lang=ru). Чтобы включить карту в витрине, необходимо задать публичный ключ API через переменные окружения.
...
ApiShip требует номер телефона получателя для расчета стоимости доставки и создания заказа в ApiShip. По этой причине витрина должна требовать указания номера телефона в форме адреса доставки.
...
Пометив поле ввода номера телефона как `required`, система оформления заказа всегда собирает его в начале процесса, обеспечивая наличие необходимых данных для расчета стоимости доставки ApiShip и запросов на создание отправления.

## 2. Обработка данных корзины

Для интеграции ApiShip в процесс оформления заказа, витрина должна иметь возможность сохранять выбор клиента по доставке в корзине и извлекать его позже. Это обеспечивает стабильность процесса при обновлении страницы и навигации. Если клиент уже выбрал тариф и пункт самовывоза, витрина сможет восстановить этот выбор, вместо того чтобы просить его выбрать заново. Для интеграция также необходимо действие по удалению данных, которое очищает выбор не только визуально, но и в самой корзине.
...
Для правильного отображения вариантов доставки ApiShip на этапе доставки, витрина должна иметь доступ к динамическим данным калькульляций, информации о пунктах самовывоза и списку доступных поставщиков. Помимо тарифов и пунктов самовывоза, витрина также получает метаданные поставщиков, чтобы отображать в интерфейсе оформления заказа понятные для человека названия перевозчиков вместо внутренних идентификаторов.
...
Кроме того, витрина загружает список поставщиков ApiShip, чтобы отображать в интерфейсе удобные для человека названия перевозчиков.

## 4. Определения типов

Интеграция с витриной основана на небольшом наборе общих типов, которые описывают данные, возвращаемые конечной точкой вычисления ApiShip, и данные, сохраняемые в корзине в качестве выбора пользователя по доставке. Эти типы используются в интерфейсе карты, модальных окнах курьеров и пунктов самовывоза, а также в компоненте выбранного тарифа для обеспечения согласованности потока и типовой безопасности.
...
`ApishipCalculation` представляет рассчитанные варианты доставки, возвращаемые API store, разделенные на группы `to door` и `to point` по поставщикам. `Chosen` — это нормализованная полезная нагрузка, которую витрина сохраняет в корзине после того, как пользователь выбирает тариф и пункт самовывоза.

## 5. Утилиты

Интеграция с витриной использует набор общих утилит для обеспечения единообразия пользовательского интерфейса и предотвращения дублирования логики в компонентах. Эти утилиты обрабатывают форматирование времени доставки, стабильные ссылки обратного вызова для асинхронных эффектов, детерминированные ключи тарифов для выбора, блокировку прокрутки для модальных окон и отображение тарифов пунктов самовывоза в структуре, которую легко визуализировать.
...
Этот компонент отвечает за отображение карты, размещение на ней маркеров пунктов самовывоза и показ панели с подробной информацией о доступных тарифах при выборе пункта.
...
Этот компонент инициализирует Yandex Maps один раз, перецентрирует карту при изменении списка пунктов самовывоза и отображает кликабельные маркеры для каждого доступного пункта. Когда пользователь выбирает пункт, в витрине появляется боковая панель, на которой отображаются тарифы и можно выбрать один из них. Пользовательский интерфейс карты также учитывает ранее сохраненный выбор. Если пользователь снова открывает модальное окно, компонент выделяет уже выбранный пункт самовывоза и тариф и не позволяет повторно выбрать тот же вариант.
...
После этого витрина может отображать ранее сохраненный выбор непосредственно на этапе доставки, включая пункт самовывоза и тарифные детали, если применимо, и позволить пользователю либо удалить его, либо вновь открыть модальное окно, чтобы изменить его.
...
Все компоненты и вспомогательные утилиты ApiShip экспортируются через общий файл, который действует как единая точка входа для интеграции. Он повторно экспортирует основные строительные блоки пользовательского интерфейса вместе с общими типами и вспомогательными утилитами, чтобы другие части витрины могли импортировать все из одного места, а не ссылаться на внутренние пути к файлам.
...
Это позволяет сохранить чистоту и согласованность импортов, а также упрощает обслуживание и рефакторинг интеграции в будущем без изменения путей импорта в витрине.
