...
## 1. Require Recipient Phone Number
...
## 2. Cart Data Handling for ApiShip Integration

To integrate ApiShip into checkout, the storefront needs a way to save the customer’s delivery choice into the cart and read it back later. This makes the flow stable across refreshes and navigation: if a customer already picked a tariff and a pickup point, the storefront can restore that selection instead of asking them to choose again. The integration also needs a proper remove action that clears the selection not only visually, but in the cart itself.
...
These changes enable three core behaviors: the storefront can attach ApiShip-specific selection data when applying a shipping method, it can read that saved selection back from the cart when the page is reloaded, and it can fully clear the selection by removing the shipping method from the cart when the customer decides to reset their choice.

## 3. ApiShip Data Fetching
...
The storefront integration relies on a small set of shared types that describe the data returned by the ApiShip calculation endpoint and the data persisted into the cart as the user’s selection. These types are used across the map UI, courier and pickup points modals, and the chosen component to keep the flow consistent and type-safe.
...
`ApishipCalculation` represents the calculated delivery options returned by the backend, separated into `to door` and `to point` groups by provider. `Chosen` is the normalized payload that the storefront saves into the cart once the user picks a tariff and a pickup point.

## 5. Utilities

The storefront integration uses a set of shared utilities to keep the UI consistent and to avoid duplicating logic across the components. These helpers handle delivery time formatting, stable callback references for async effects, deterministic tariff keys for selection, scroll locking for modals, and mapping pickup point tariffs into a structure that is easy to render.
...
These utilities are referenced by the modals and the map to format delivery times, keep selection keys stable between renders, safely run async effects without state updates after unmount, and correctly prepare the pickup point data model used to render tariffs per point.
...
This component initializes Yandex Maps v3 once, re-centers the map when the pickup point list changes, and renders clickable markers for each available point. When the user selects a point, the storefront shows a side panel where tariffs are displayed and can be chosen. The map UI also respects the previously saved selection: if the user opens the modal again, the component highlights the already chosen pickup point and tariff and prevents re-selecting the same option.

## 7. Selected ApiShip Delivery Summary
...
With this in place, the storefront can show the previously saved selection directly in the delivery step, including pickup point and tariff details when applicable, and let the user either remove it or reopen the modal to change it.

## 8. Pickup Point Modal
...
![Directory structure in the Medusa Storefront after creating the file for apiship pickup point modal](https://github.com/user-attachments/assets/ffb59344-83e4-4b02-a437-58d4660e2e63)
...
When the modal is opened, it restores the previously selected tariff (if one was already saved in the cart), so the user sees their earlier choice immediately. All available courier tariffs are displayed in a grouped layout, and the customer can select one using radio buttons.
...
![Directory structure in the Medusa Storefront after creating the file for apiship courier modal](https://github.com/user-attachments/assets/cbe90ee1-11af-46ce-bdb3-5eee32617c34)
...
Once the customer confirms their choice by clicking `Choose`, the selected tariff is saved to the cart, the shipping price is recalculated, and the modal closes. If the previously selected tariff is already active, the interface reflects that state accordingly, so the user clearly sees their current selection.

## 10. Public Exports

All ApiShip storefront components and helpers are exposed through a shared file that acts as a single entry point for the integration. It re-exports the main UI building blocks, along with shared types and utility helpers, so other parts of the storefront can import everything from one place instead of referencing internal file paths.
...
To support ApiShip calculated delivery methods, the shipping step needs an extra UI flow where the customer chooses a concrete tariff, and optionally a pickup point. This update adds ApiShip-specific state and modals, loads provider names for display, and saves the final selection into the cart so the checkout can proceed only after the customer has made a valid choice.
...
This change adds two dedicated modals for ApiShip (pickup point selection and courier tariff selection) and a small summary block that shows the currently saved choice with `Edit` and `Remove` actions. It also ensures that the customer can’t continue to payment for an ApiShip shipping option until a valid ApiShip selection has been saved into the cart, and that ApiShip calculated prices are displayed as `from X` to reflect that the final amount depends on the selected tariff.
