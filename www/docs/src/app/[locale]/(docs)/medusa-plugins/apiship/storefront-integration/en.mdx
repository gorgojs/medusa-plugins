...
To integrate the Apiship fulfillment provider in a Next.js storefront, you need to extend the checkout shipping step with support for pickup-point delivery.

Apiship pickup delivery differs from regular shipping methods in that it requires additional data to be collected during checkout, such as the selected pickup point and delivery tariff. This data must be loaded dynamically from the backend, presented to the customer in a convenient form, and then persisted in the cart so it can be used later by the fulfillment provider when creating a orders in the Apiship.
...
## 1. Shipping method metadata support

To support Apiship pickup delivery, the storefront needs a way to attach provider-specific metadata to a shipping method. This metadata is used to store delivery-related information that cannot be derived from the shipping option alone, such as the selected pickup point and tariff.
...
This metadata is later read by the Apiship fulfillment provider during order creation, allowing it to correctly create an order in the Apiship using the pickup point and tariff selected during checkout.

## 2. Pickup calculation and points requests

To render pickup delivery options in the shipping step, the storefront needs access to Apiship calculation data and pickup point details. This information is not static and must be requested dynamically based on the current cart and selected shipping option.
...
Apiship requires the recipient’s phone number for delivery calculations and for creating a order in the Apiship. For this reason, the storefront must enforce a phone number in the shipping address form.
...
To integrate Apiship pickup delivery into the shipping step, the storefront needs a component that loads pickup points and tariffs for the selected shipping option, tracks the current selection, and persists the final choice into the cart.
...
To enable Apiship pickup delivery in checkout, the shipping step must detect when an Apiship pickup shipping option is selected, render the pickup selection UI, and control the step completion state based on whether the selection is saved to the cart.
...
This integration adds an `apishipReady` state that is updated by `ApishipWrapper` through `onReadyChange`. When an Apiship pickup option is active and the selection is not yet completed, the shipping step disables the `Continue to payment` button. This ensures the cart always contains the required Apiship pickup metadata before moving to the payment step.
