...
Присоединяйтесь к [Medusa.js ⊷ T-Kassa (T-Bank)](https://t.me/medusajs_tkassa) чату сообщества, чтобы обсудить новые функции и получить поддержку.
...
💳 Поддержка **одностадийных** (автосписание) и **двухстадийных** (авторизация/холд) сценариев оплаты  
📊 **Webhook-уведомления** о статусе платежа в реальном времени  
🔄 **Полные возвраты** и **отмена заказов**  
🛡 **Проверка webhook по токену** для повышения безопасности  
...
Добавьте конфигурацию провайдера в файл `medusa-config.js` вашего Medusa-проекта:
...
| `capture`           | Автосписание:<br>- `true` — одностадийная оплата (`O` в API)<br>- `false` — двухстадийная оплата (`T` в API)     | `true`       |
| `useReceipt`        | Включить формирование чеков по ФФД                                                                               | `false`      |
| `ffdVersion`        | Версия ФФД: "1.2" или "1.05" <br> Применимо только если `useReceipt=true`                                    | -            |
| `taxation`          | Система налогообложения:<br>- `osn` — ОСН<br>- `usn_income` — УСН (доходы)<br>- `usn_income_outcome` — УСН (доходы-расходы)<br>- `esn` — ЕСХН<br>- `patent` — Патент<br>Применимо только если `useReceipt=true` | - |
| `taxItemDefault`    | Ставка НДС по товарам:<br>- `none` — без НДС<br>- `vat0` — 0%<br>- `vat5` — 5%<br>- `vat7` — 7%<br>- `vat10` — 10%<br>- `vat20` — 20%<br>- `vat105` — 5/105<br>- `vat107` — 7/107<br>- `vat110` — 10/110<br>- `vat120` — 20/120<br>Применимо только если `useReceipt=true` | - |
| `taxShippingDefault`| Ставка НДС для доставки (аналогично `taxItemDefault`)<br>Применимо только если `useReceipt=true`                 | -            |

## Интеграция со storefront

Чтобы интегрировать T-Kassa в storefront Next.js, добавьте необходимые UI-компоненты, чтобы провайдер отображался на странице checkout.
...
Чтобы сделать T-Kassa доступным в качестве способа оплаты на странице оформления заказа в магазине, вы должны добавить его конфигурацию к сопоставлению поставщиков платежей в файле констант вашего магазина. Это сопоставление определяет, как каждый поставщик платежей отображается в пользовательском интерфейсе.
...
![Структура каталогов в storefront магазина Medusa после обновления файла с константами](https://github.com/user-attachments/assets/0aee001e-958f-40c6-b329-618e318ff019)
...
Вы расширяете объект `paymentInfoMap`, добавляя в него запись `pp_tkassa_tkassa`. Эта запись определяет заголовок и значок, которые будут отображаться для T-Kassa на странице оформления заказа.

Вспомогательная функция `isTkassa` проверяет, принадлежит ли данный `providerId` T-Kassa. Это полезно при отображении инструкций или компонентов, относящихся к конкретному провайдеру.
...
![Структура каталогов в storefront магазина Medusa после обновления файла для файлов cookie](https://github.com/user-attachments/assets/4274d249-6994-4d9f-b4b6-98f2016f0e9f)
