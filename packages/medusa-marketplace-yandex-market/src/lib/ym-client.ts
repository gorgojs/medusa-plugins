...
export function warehousesApi(credentials: MarketplaceYandexMarketCredentialsType) {
  const { api_key } = assertValidCredentials(credentials)
  return new WarehousesApi(new Configuration({ apiKey: api_key }))
}

export function dbsApi(credentials: MarketplaceYandexMarketCredentialsType) {
  const { api_key } = assertValidCredentials(credentials)
  return new DbsApi(new Configuration({ apiKey: api_key }))
}

export function campaignApi(credentials: MarketplaceYandexMarketCredentialsType) {
  const { api_key } = assertValidCredentials(credentials)
  return new CampaignsApi(new Configuration({ apiKey: api_key }))
}

export function DbsApiAxiosParamCreator(credentials: MarketplaceYandexMarketCredentialsType) {
  const { api_key } = assertValidCredentials(credentials)
  return new CampaignsApi(new Configuration({ apiKey: api_key }))
}