...
import { TAX, requiredWhenReceipt, validateNotifications } from "../utils"
...
const descriptor = defineIntegration({
  ...
  options: {
    ...
    // Opaque JSON (array of `{ event, url }`) validated imperatively; nested secrets are NOT encrypted.
    notifications: {
      type: "json",
      default: [{ event: "all", url: "url" }],
      control: "json",
      label: "tkassa.fields.notifications",
      hint: "tkassa.hints.notifications",
      validate: validateNotifications,
    },
    // Catalog-only (no section references it) — a custom-section widget owns its UI and saves it.
    webhookUrl: {
      type: "url",
      label: "tkassa.fields.webhookUrl",
      hint: "tkassa.hints.webhookUrl",
      protocols: ["https"],
    },
    // Readonly: disabled in the UI; its `default` is displayed and persisted.
    webhookEndpoint: {
      type: "url",
      label: "This is readonly",
      readonly: true,
  ...
  sections: [
    ...
    { id: "behavior", title: "tkassa.sections.behavior", column: "side", options: ["capture", "webhookEndpoint"] },
    { id: "receipt", title: "tkassa.sections.receipt", options: ["useReceipt", "ffdVersion", "taxation", "taxItemDefault", "taxShippingDefault"] },
    { id: "advanced", title: "tkassa.sections.advanced", column: "side", options: ["notifications"] },
  ],
