...
describe("notifications (json option validated imperatively)", () => {
  const descriptor = new TkassaIntegrationProvider().descriptor
  const base = { terminalKey: "t", password: "p" } // minimal valid, receipts off

  it("accepts an empty array (default)", () => {
    const issues = collectValidationIssues(descriptor as any, { ...base })
    expect(issues.some((i) => i.path.startsWith("notifications"))).toBe(false)
  })
  it("rejects a non-array notifications", () => {
    const issues = collectValidationIssues(descriptor as any, { ...base, notifications: "nope" })
    expect(issues.some((i) => i.path.startsWith("notifications"))).toBe(true)
  })
  it("rejects an item with a bad event / empty url", () => {
    const issues = collectValidationIssues(descriptor as any, {
      ...base,
      notifications: [{ event: "bogus", url: "" }],
    })
    expect(issues.length).toBeGreaterThan(0)
  })
  it("accepts well-formed notifications", () => {
    const issues = collectValidationIssues(descriptor as any, {
      ...base,
      notifications: [{ event: "payment", url: "https://x.io/hook" }],
    })
    expect(issues.some((i) => i.path.startsWith("notifications"))).toBe(false)
  })
})
