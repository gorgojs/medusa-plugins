...
**customer_price** | **number** | Цена товара для покупателя с учётом скидок продавца и Ozon. | [optional] [default to undefined]
...
```typescript
import { PostingFinancialDataProduct } from './api';

const instance: PostingFinancialDataProduct = {
    actions,
    currency_code,
    commission_amount,
    commission_percent,
