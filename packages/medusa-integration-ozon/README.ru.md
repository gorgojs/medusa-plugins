...
<p align="center">
  ...
  <br/>
  <a href="https://github.com/gorgojs/medusa-plugins/blob/HEAD/packages/medusa-integration-ozon/README.md">Read README in English ↗</a>
</p>
...
Установите core-плагин Integration и плагин-провайдер Ozon:
...
module.exports = defineConfig({
  ...
  plugins: [
    ...
    // Регистрация оcore-плагина integration и объявление провайдера Ozon
    {
...
| `providers[].resolve` | `string` | Yes          | Путь к модулю провайдера. Для Ozon: `"@gorgo/medusa-integration-ozon/providers/integration-ozon"`                                        |
...
**Параметры плагина `@gorgo/medusa-integration-ozon`:**

Передача опций на уровне регистрации плагина не требуется. Все настройки интеграции (например, API‑ключ) задаются отдельно для каждого подключения в Medusa Admin.

**Параметры Vite‑плагина `gorgoPluginsInject`:**

| Параметр  | Тип       | Описание                                                                                                                                                   |
| --------- | ----------| ---------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `sources` | `string[]`| Список пакетов Gorgo‑плагинов, чьи расширения админ‑интерфейса должны быть внедрены в Medusa Admin. Укажите все установленные `@gorgo/medusa-integration-*` |
...
Эта документация описывает, как управлять интеграциями с Ozon из панели управления Medusa.

### Управление интеграциями

В этом разделе вы узнаете, как управлять интеграциями в Medusa Admin.

#### Просмотр списка интеграций

Перейдите в **Settings → Integrations**, чтобы увидеть таблицу всех настроенных интеграций.
...
| Колонка         | Описание                                                                 |
| --------------- | ------------------------------------------------------------------------ |
| **Title**       | Отображаемое имя, которое вы задали интеграции                        |
| **Provider**    | Тип интеграции (например, `ozon`)                                     |
| **Sales Channel** | Канал продаж Medusa, связанный с этой интеграцией                   |
| **Status**      | Включёна или выключена данная интеграция                                 |
...
#### Просмотр деталей интеграции

Нажмите на интеграцию в списке, чтобы открыть страницу его настроек. Страница разделена на несколько секций:

- **General** — название и статус включения.
- **Exchange Profiles** — привязка складов и типов заказов.
- **Events** — журнал всех операций синхронизации для этой интеграции.
...
1. На странице интеграции найдите секцию **General**.
...
Секция **Credentials** добавляется виджетом `@gorgo/medusa-integration-ozon` на странице интеграции.

1. Найдите секцию **Credentials** на странице интеграции.
...
1. Найдите секцию **Exchange settings** на странице интеграции.
...
1. Перейдите в **Settings → Integrations** и выберите интеграцию Ozon.
...
> При включённой интеграции в Ozon выгружаются только товары из тех категорий Medusa, для которых настроен маппинг. Пэйлоады формируются на основе выбранных категорий и настроек маппинга атрибутов.
...
1. На странице интеграции нажмите **Synchronize**.
...
Синхронизация товаров выполняется автоматически каждый день в полночь (UTC) фоновым заданием `sync-integration-products`. Дополнительная настройка не требуется.
...
Синхронизация заказов выполняется автоматически каждый день в полночь (UTC) фоновым заданием `sync-integration-orders`. Дополнительная настройка не требуется.

---

#### Удаление интеграции

1. На странице интеграции откройте меню действий.
2. Нажмите **Delete**.
3. Подтвердите удаление.

![settings.marketplace.[id].sync-orders](../../www/docs/public/static/marketplace-ozon/image-10.png)

> При удалении интеграции также безвозвратно удаляются все связанные профили обмена и события.

---

#### Просмотр событий

Перейдите в **Settings → Integrations → Events**, чтобы увидеть журнал всех операций синхронизации по всем интеграциям. События, относящиеся к одной интеграции, также отображаются на его странице.
...
| Колонка       | Описание                                                                                       |
| ------------- | ---------------------------------------------------------------------------------------------- |
| **Direction** | Направление: `Medusa → Integration` (экспорт) или `Integration → Medusa` (импорт)             |
