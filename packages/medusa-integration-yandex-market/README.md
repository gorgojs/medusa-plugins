...
<p align="center">
  ...
  <br/>
  <a href="https://github.com/gorgojs/medusa-plugins/blob/HEAD/packages/medusa-integration-yandex-market/README.ru.md">Читать README на русском ↗</a>
</p>
...
- 🧩  **Built as a provider on top of [`@gorgo/medusa-integration`](https://www.npmjs.com/package/@gorgo/medusa-integration)** with shared admin UI, events & workflows
...
- 🛠  **Admin UI** for managing integrations, credentials & configs
...
- [@gorgo/medusa-integration](https://www.npmjs.com/package/@gorgo/medusa-integration) core plugin

## Installation

Install both the Integration core plugin and the Yandex Market provider:

```bash
npm install @gorgo/medusa-integration @gorgo/medusa-integration-yandex-market

# or
yarn add @gorgo/medusa-integration @gorgo/medusa-integration-yandex-market
```
...
module.exports = defineConfig({
  ...
  plugins: [
    ...
    // Register the integration core plugin and declare Yandex Market as a provider
    {
...
| `providers[].resolve` | `string` | Yes      | Path to the provider module. For Yandex Market: `@gorgo/medusa-integration-yandex-market/providers/integration-yandex-market`. |
...
**`@gorgo/medusa-integration-yandex-market` plugin options:**

No options are required at the plugin level. All integration-specific settings (such as the API key) are configured per-integration instance in the admin UI.
...
In this guide, you’ll learn how to manage integrations in the Medusa Admin.

#### View Integrations

Go to **Settings → Integrations** to see a table of all configured integration integrations.
...
| Column            | Description                                          |
| ----------------- | ---------------------------------------------------- |
| **Title**         | The display name you gave to the integration.        |
| **Provider**      | The integration type (e.g. `yandexmarket`).           |
| **Sales Channel** | The Medusa sales channel linked to this integration. |
| **Status**        | Whether the integration is enabled or disabled.      |
...
2. Click **Add integration** to create a integration connection.
3. Fill in the form:
   - **Title** — a human-readable name for this integration connection (e.g. "Yandex Market Store").
   - **Provider** — select `yandexmarket` from the dropdown.
   - **Sales Channel** — select the Medusa sales channel that products and orders for this integration will belong to.
   - **Enabled** — toggle whether this integration is active.
...
Click on a integration in the list to open its detail page. The detail page is divided into several sections:

- **General** — title and enabled status.
- **Exchange Profiles** — warehouse and order type mappings.
- **Events** — a log of all sync operations for this integration.
...
1. On the integration detail page, find the **General** section.
...
The **Credentials** section is provided by the `@gorgo/medusa-integration-yandex-market` widget on the integration detail page.

1. Find the **Credentials** section on the integration detail page.
...
### Manage Integration Exchange Settings

In this guide, you’ll learn how to manage Yandex Market credentials in the Medusa Admin.

#### Edit Integration Exchange Settings

Exchange settings map a **warehouse** to an **order type** (FBS, FBO, DBS, ...). It is required to sync orders.

1. Find the **Exchange settings** section on the integration detail page.
...
| ---------------------- | ----------------------------------------------------- |
| `yandex_market_offer_id`     | integration offer id; in this integration it matches the offerId used on export and typically matches the Medusa product_variant.id.  |
| `yandex_market_card_status`    | Card status from the integration (e.g. NO_CARD_ERRORS). |
...
1. On the integration detail page, click "Synchronize".
...
Products are automatically synced every day at midnight (UTC) via the `sync-integration-products` background job. No additional configuration is needed.
...
Orders are automatically synced every day at midnight (UTC) via the `sync-integration-orders` background job. No additional configuration is needed.

---

#### Delete Integration

1. On the integration detail page, open the actions menu.
2. Click "Delete".
3. Confirm the deletion.

![settings.marketplaces.[id].delete](../../www/docs/public/static/marketplace-yandex-market/image-9.png)

> Deleting a integration also permanently deletes all associated exchange profiles and events.

---

#### View Events

Go to **Settings → Integrations → Events** to see a log of all sync operations across all integrations. Events scoped to a single integration are also visible on its detail page.
...
| Column        | Description                                                                                |
| ------------- | ------------------------------------------------------------------------------------------ |
| **Direction** | `Medusa → Integration` (export) or `Integration → Medusa` (import).                        |
