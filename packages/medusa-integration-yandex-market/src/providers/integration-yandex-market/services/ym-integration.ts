...
import {
  exportIntegrationProductsYmWorkflow,
...
import { IntegrationYandexMarketCredentialsType, ORDER_TYPES } from "../types"
...
export class YandexMarketIntegrationProvider extends AbstractIntegrationProvider {
  static identifier = "yandexmarket"

  async exportProducts(data: ExportProductsInput): Promise<ExportProductsOutput> {
    const { container, integrationProducts, integration } = data
    const products = (integrationProducts as { integrationProducts?: UpdateOfferMappingDTO[] }).integrationProducts ?? []

    const { result } = await exportIntegrationProductsYmWorkflow(container!).run({
      input: {
        credentials: integration.credentials as IntegrationYandexMarketCredentialsType,
        products
      },
    })
  ...
  async getIntegrationProducts(data: GetIntegrationProductsInput): Promise<GetIntegrationProductsOutput> {
    ...
    const credentials = integration.credentials as IntegrationYandexMarketCredentialsType
    ...
    if (ids?.length) {
      ...
      return result as GetIntegrationProductsOutput
    }
  ...
  async mapToMedusaProducts(input: MapToMedusaProductsInput): Promise<MapToMedusaProductsOutput> {
    const { container, integration, integrationProducts } = input
    const offerMappings = Array.isArray(integrationProducts) ? (integrationProducts as GetOfferMappingDTO[]) : []
  ...
  async mapToIntegrationProducts(data: MapToIntegrationProductsInput): Promise<MapToIntegrationProductsOutput> {
    const settings = {
      "medusaToIntegrationMappingSchema": {
    ...
    const schema = settings.medusaToIntegrationMappingSchema
    const products = data.products ?? []
    const integrationProducts: any[] = []
    ...
    products.forEach((product) => {
      ...
      product.variants.forEach((variant) => {
        ...
        integrationProducts.push({ offer: offer })
      })
    })
    return {
      integrationProducts
  ...
  async getIntegrationWarehouses(data: GetIntegrationWarehousesInput): Promise<GetIntegrationWarehousesOutput> {
    ...
    const api = warehousesApi(integration.credentials)

    const limit = 30
    let pageToken: string | undefined
    const result: GetIntegrationWarehousesOutput = []

    do {
      const response = await api.getPagedWarehouses(
        withBusinessId(integration.credentials, {
          pageToken,
          limit,
          getPagedWarehousesRequest: {
            campaignIds: [Number(integration.credentials.campaign_id)],
          } as any
        })
      )
  ...
  async getIntegrationOrders(data: GetIntegrationOrdersInput): Promise<GetIntegrationOrdersOutput> {
    ...
    return orders as unknown as GetIntegrationOrdersOutput
  }
  ...
  async mapToMedusaOrders(data: MapToMedusaOrdersInput): Promise<MapToMedusaOrdersOutput> {
    ...
    const ymOrders = integrationOrders as unknown as BusinessOrderDTO[]
    ...
    const MedusaOrders = ymOrders.map(order => {
      ...
      console.log(mapOrder.integration_order.type);
      return mapOrder
    })
