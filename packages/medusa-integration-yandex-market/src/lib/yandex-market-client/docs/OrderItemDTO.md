...
**buyerPriceBeforeDiscount** | **number** | Стоимость товара в валюте покупателя до применения скидок по:  * акциям; * купонам; * промокодам.  Это зачеркнутая цена, которая отображается покупателю на карточке товара до применения скидок.  | [default to undefined]
...
**partnerWarehouseId** | **string** | **Только для моделей FBY и LaaS**  Идентификатор склада, на который сформирован заказ.  | [optional] [default to undefined]
**promos** | [**Array&lt;OrderItemPromoDTO&gt;**](OrderItemPromoDTO.md) | Информация о вознаграждении продавцу за скидки на товар по промокодам, купонам и акциям. | [optional] [default to undefined]
