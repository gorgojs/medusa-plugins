...
**warehouseId** | **number** | Идентификатор фулфилмент-склада Маркета.  Передайте этот параметр, чтобы вернулись варианты доставки для указанного склада. Иначе Маркет сам выберет склад.  [Как узнать остатки товаров на складах](../../reference/stocks/getStocks.md)  | [optional] [default to undefined]

## Example

```typescript
import { GetDeliveryOptionsItemDTO } from './api';

const instance: GetDeliveryOptionsItemDTO = {
    offerId,
    count,
    warehouseId,
};
