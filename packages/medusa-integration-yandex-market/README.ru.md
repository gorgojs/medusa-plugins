...
<p align="center">
  ...
  <br/>
  <a href="https://github.com/gorgojs/medusa-plugins/blob/HEAD/packages/medusa-integration-yandex-market/README.md">Read README in English ↗</a>
</p>
...
Установите core-плагин Integration и плагин-провайдер Yandex Market:
...
module.exports = defineConfig({
  ...
  plugins: [
    ...
    // Регистрация core-плагина integration и объявление провайдера Yandex Market
    {
...
| --------------------- | -------- | ----------- | ----------------------------------------------------------------------------------------------------------------------------- |
| `providers`           | `array`  | Да          | Список регистраций провайдеров интеграций.                                                                                 |
| `providers[].resolve` | `string` | Да          | Путь к модулю провайдера. Для Yandex Market: `@gorgo/medusa-integration-yandex-market/providers/integration-yandex-market`. |
...
**Параметры плагина `@gorgo/medusa-integration-yandex-market`:**

Указание параметров на уровне регистрации плагина не требуется. Все настройки интеграции (например, API-ключ) задаются отдельно для каждого подключения в Medusa Admin.
...
| **Provider**      | Тип интеграции (например, `yandexmarket`)          |
...
2. Нажмите **Add integration**, чтобы создать подключение к интеграции.
3. Заполните форму:
...
Нажмите на интеграцию в списке, чтобы открыть страницу деталей. На ней несколько разделов:

- **General** — название и статус включения.
- **Exchange Profiles** — привязка складов к типам заказов.
- **Events** — журнал операций синхронизации для этой интеграции.
- **Credentials** — API-ключ и данные Yandex Market (виджет провайдера `@gorgo/medusa-integration-yandex-market`).

![settings.marketplaces.[id]](../../www/docs/public/static/marketplace-yandex-market/image-3.png)

---

#### Редактирование карточки интеграции

1. На странице интеграции найдите раздел **General**.
...
Раздел **Credentials** добавляется виджетом `@gorgo/medusa-integration-yandex-market` на странице интеграции.

1. Откройте раздел **Credentials** на странице интеграции.
...
1. На странице интеграции откройте раздел **Exchange settings**.
...
Товары автоматически синхронизируются каждый день в полночь (UTC) фоновой задачей `sync-integration-products`. Дополнительная настройка не требуется.
...
Заказы автоматически синхронизируются каждый день в полночь (UTC) задачей `sync-integration-orders`. Дополнительная настройка не требуется.
...
> При удалении интеграции безвозвратно удаляются связанные профили обмена и события.

---

#### Просмотр событий

Перейдите в **Settings → Integrations → Events**, чтобы увидеть журнал синхронизаций по всем интеграциям. События конкретной интеграции также доступны на его странице.
