...
import { flattenClassifierGroups } from "../../../lib/product-utils"
import { ImportOutput } from "../../../types"
import { processCategoriesRecursivelyStep } from "../steps/process-categories"
import { parseCommercemlImportStep } from "../steps/parse-commerceml-import"

export type ImportMarketplaceProductsOneCWorkflowInput = {
  import: string[]
}

export const importMarketplaceProductsWorkflow = createWorkflow(
  "mp-1c-import-products-workflow",
  (input: ImportMarketplaceProductsOneCWorkflowInput) => {
    const importData = parseCommercemlImportStep(input.import)
    ...
    const externalCategoriesIdsFilters = transform(
      { onecData: importData },
      (data) =>
    ...
    const { data: existingCategories } = useQueryGraphStep({
      ...
      filters: {
        $or: [
          { metadata: { onec_id: externalCategoriesIdsFilters } },
