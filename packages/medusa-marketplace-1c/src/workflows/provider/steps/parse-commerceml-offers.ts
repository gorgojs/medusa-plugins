...
import { BitrixCommerceMlOffersParser } from "../../../lib/bitrix-offers-parser"

export const parseCommercemlOffersStep = createStep(
  "mp-1c-parse-offer-files",
  async (filePaths: ParseFileStepInput, { container }) => {
    ...
    try {
      if (filePaths) {
        logger.info(
          `[1C Marketplace] Parsing offer files: ${filePaths.join(", ")}`
        )
        await Promise.all(
        ...
        logger.info(`[1C Marketplace] Parsed offers data successfully.`)
      }
    } catch (e) {
      logger.error(
        `[1C Marketplace] Failed to parse offer file: ${(e as Error).message}`
      )
      throw e
    }
