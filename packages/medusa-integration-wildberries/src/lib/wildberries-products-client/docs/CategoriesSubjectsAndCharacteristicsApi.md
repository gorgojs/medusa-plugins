...
|------------- | ------------- | -------------|
|[**apiContentV1BrandsGet**](#apicontentv1brandsget) | **GET** /api/content/v1/brands | Brands|
...
# **apiContentV1BrandsGet**
> BrandsResponse apiContentV1BrandsGet()

The method returns list of brands by subject ID.  <div class=\"description_limit\"> <a href=\"/openapi/api-information#tag/Introduction/Rate-Limits\">Request limit</a> per one seller\'s account:  | Period | Limit | Interval | Burst | | --- | --- | --- | --- | | 1 s | 1 request | 1 s | 5 requests | </div> 
...
let next: number; //Pagination parameter. Use the `next` value from the response to get the next data batch (optional) (default to undefined)

const { status, data } = await apiInstance.apiContentV1BrandsGet(
    subjectId,
    next
);
```
...
| **next** | [**number**] | Pagination parameter. Use the &#x60;next&#x60; value from the response to get the next data batch | (optional) defaults to undefined|
